...
#plotting
lapply(X = cntrl_densities, FUN = function(j){
...
testing_ii = cntrl_densities$`3`$`3`
testing_ij = cntrl_densities$`3`$`5`
testing_ik = cntrl_densities$`3`$`7`
...
density_comparisons_cntrl = data.frame(cromo_i = character(),
                                       cromo_j = character(),
                                       hellinger_D = numeric(),
                                       JSD_D       = numeric()
)
...
for(i in seq_along(cromos)){
  cromo_i = cromos[i]
  pdf_ii = cntrl_densities[[i]][[i]]$y
  
  for(j in seq_along(cromos)){
    cromo_j = cromos[j]
    pdf_ij = cntrl_densities[[i]][[j]]$y
    ...
    density_comparisons_cntrl = rbind(density_comparisons_cntrl,
                                      my_result)
    
  }
}

p = ggplot(data = density_comparisons_cntrl, 
           mapping = aes(cromo_i,cromo_j, fill=hellinger_D)
)


#p = ggplot(data = ks_cntrl_ij, mapping = aes(i,j, fill=(ks.d)))
p = p + geom_raster()
...
head(density_comparisons_cntrl)

cntrl_densities[[1]][[2]]
cntrl_densities[[2]][[1]]

density_comparisons_cntrl%>%
  filter(cromo_i == 1 & cromo_j == 2)
density_comparisons_cntrl%>%
  filter(cromo_i == 2 & cromo_j == 1)

########################################
cntrl_ijkl =              data.frame(i = character(),
                                     j = character(),
                                     k = character(),
                                     l = character(),
                                     hellinger_D = numeric(),
...
for(i in seq_along(cromos)){
  for(j in seq_along(cromos)){
    pdf_ij = pdf_ij = cntrl_densities[[i]][[j]]$y
    for(k in seq_along(cromos)){
      for(l in seq_along(cromos)){
        pdf_kl = cntrl_densities[[k]][[l]]$y
        ...
        cntrl_ijkl = rbind(cntrl_ijkl, my_result)
      }
    }
  }
}

saveRDS(cntrl_ijkl, "results/cntrl_ijkl.RDS")
cntrl_ijkl%>%arrange(desc(hellinger_D))%>%head(10)
...
cntrl_ijkl%>%
  ...
  write_delim(path = "results/cntrl_cromoDistance.txt")
